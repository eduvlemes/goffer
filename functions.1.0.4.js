...
function showCardInvoiceImport() {
  // Cria modal simples
  let modal = document.getElementById('card-invoice-import-modal');
  if (!modal) {
    modal = document.createElement('div');
    modal.id = 'card-invoice-import-modal';
    ...
    modal.innerHTML = `
      <div style="background:#fff;padding:32px;max-width:700px;width:100%;border-radius:8px;box-shadow:0 2px 16px #0002;position:relative;">
        <h2>Importar Fatura de Cartão</h2>
        <textarea id="card-invoice-textarea" style="width:100%;height:180px;margin-bottom:16px;"></textarea>
        <div id="card-invoice-preview" style="max-height:300px;overflow:auto;margin-bottom:16px;"></div>
        <button id="card-invoice-parse-btn">Analisar Fatura</button>
        <button id="card-invoice-import-btn" style="display:none;margin-left:8px;">Importar Lançamentos</button>
        <button id="card-invoice-close-btn" style="position:absolute;top:8px;right:8px;">&times;</button>
      </div>
    `;
  ...
  // Fechar modal
  modal.querySelector('#card-invoice-close-btn').onclick = () => {
    modal.style.display = 'none';
  };

  // Analisar fatura
  modal.querySelector('#card-invoice-parse-btn').onclick = () => {
    const text = modal.querySelector('#card-invoice-textarea').value;
    const parsed = parseCardInvoice(text);
    renderCardInvoicePreview(parsed, modal);
    modal.querySelector('#card-invoice-import-btn').style.display = 'inline-block';
  };

  // Importar lançamentos (placeholder)
  modal.querySelector('#card-invoice-import-btn').onclick = () => {
    alert('Importação concluída! (placeholder)');
    modal.style.display = 'none';
  };
}

// Função para parsear lançamentos da fatura
function parseCardInvoice(text) {
  // Regex para capturar linhas de lançamentos: Data, Descrição, Valor
  const lines = text.split(/\r?\n/);
  const entries = [];
  const regex = /^(\d{2})\s*\/\s*(\w{3})\s+(.+?)\s+R\$\s*(-?\d+[.,]\d{2})/i;
  let currentTitular = '';
  lines.forEach(line => {
    // Detecta titular
    const titularMatch = line.match(/([\w\s]+)\s+-\s+final\s+(\d{4})/i);
    if (titularMatch) {
      currentTitular = titularMatch[1].trim() + ' - final ' + titularMatch[2];
      return;
    }
    // Detecta lançamentos
...
// Renderiza preview dos lançamentos para revisão
function renderCardInvoicePreview(entries, modal) {
  const preview = modal.querySelector('#card-invoice-preview');
  if (!entries.length) {
    preview.innerHTML = '<em>Nenhum lançamento detectado.</em>';
    return;
  }
  let html = '<table style="width:100%;border-collapse:collapse;">';
  html += '<tr style="background:#eee;"><th>Data</th><th>Descrição</th><th>Valor</th><th>Titular</th><th>Categoria</th></tr>';
  entries.forEach(e => {
    html += `<tr>
      <td>${e.dia} / ${e.mes}</td>
      <td>${e.descricao}</td>
      ...
      <td>${e.titular}</td>
      <td><input type="text" value="" placeholder="Categoria"></td>
    </tr>`;
  });
  html += '</table>';
...
const GofferUtils = {
  ...
  /**
   * Gerencia operações de localStorage
   */
  storage: {
    /**
  ...
  /**
   * Obtém todas as variações ativas/selecionadas no momento
   * @return {Array} Array com os IDs das variações ativas
   */
  getActiveVariations() {
    const activeVariations = [];
    
    // Procura por elementos com classe 'active' ou 'selected' que tenham data-variacao-id
    const activeElements = document.querySelectorAll('[data-variacao-id].active, [data-variacao-id].selected, [data-variacao-id].ativo');
    
    activeElements.forEach(element => {
      const variacaoId = GofferUtils.getData(element, 'variacaoId');
      if (variacaoId && !activeVariations.includes(variacaoId)) {
        activeVariations.push(variacaoId);
      }
    });
    
    // Se não encontrou nenhuma ativa através das classes, busca por outras formas
    if (activeVariations.length === 0) {
      // Procura por links de variação que possam estar ativos de outras formas
      const variationLinks = document.querySelectorAll('a[data-variacao-id]');
      variationLinks.forEach(link => {
        // Verifica se o link tem alguma indicação de estar ativo
        if (link.classList.contains('active') || 
            link.classList.contains('selected') || 
            link.classList.contains('ativo') ||
            link.parentElement?.classList.contains('active') ||
            link.parentElement?.classList.contains('selected') ||
            link.parentElement?.classList.contains('ativo')) {
          const variacaoId = GofferUtils.getData(link, 'variacaoId');
          if (variacaoId && !activeVariations.includes(variacaoId)) {
            activeVariations.push(variacaoId);
          }
        }
  ...
  /**
   * Verifica se uma customização deve estar visível baseada nas variações ativas
   * @param {HTMLElement} customization - Elemento da customização
   * @param {Array} activeVariations - Array com IDs das variações ativas
   * @return {boolean} True se deve estar visível
   */
  shouldCustomizationBeVisible(customization, activeVariations) {
    // Se tem a classe 'enabled-all', sempre está visível
    if (customization.classList.contains('enabled-all')) {
      return true;
    }
    
    // Verifica se alguma das variações ativas tem uma classe correspondente na customização
    return activeVariations.some(variacaoId => 
  ...
  renderCustomizer() {
    ...
    this.config.customizations.forEach(customization => {
      ...
      const customizationHtml = `
        <div data-customization-title="${customization.title}" class="customization ${customization.enableOnVariants.length > 0 ? 'hide' : 'enabled-all'} ${enabledClasses}">
  ...
  setupEvents() {
    ...
    // Evento para botão comprar
      document.addEventListener('click', (event) => {
        ...
        // Validação de customizações obrigatórias
        let hasError = false;
        let errorMessage = '';
        // Remove avisos anteriores
        this.customizerElement.querySelectorAll('.customization-message').forEach(el => el.remove());

        // Para cada customização visível
        this.customizerElement.querySelectorAll('.customization:not(.hide)').forEach(customizationEl => {
          // Busca config da customização
          const title = customizationEl.querySelector('.customization-title').textContent;
          const config = this.config.customizations.find(c => c.title === title);
          if (config && config.type === 'required') {
            // Se não tem nenhuma opção selecionada
        ...
        if (hasError) {
          ...
          const firstError = this.customizerElement.querySelector('.customization-message');
          if (firstError) {
        ...
        // Redireciona após um pequeno delay para garantir que o localStorage foi atualizado
        setTimeout(() => {
    ...
    document.addEventListener('click', (event) => {
      ...
      // Aguarda um pequeno delay para garantir que as classes .active sejam atualizadas
      setTimeout(() => {
        // Obtém TODAS as variações que estão ativas no momento
        const activeVariations = GofferUtils.getActiveVariations();
        
        // Atualiza a visibilidade das customizações baseado em TODAS as variações ativas
        this.customizationContainers.forEach(customization => {
          const shouldBeVisible = GofferUtils.shouldCustomizationBeVisible(customization, activeVariations);
        ...
        // Para verificar personalizações salvas, usa a primeira variação ativa
        const primaryVariacao = activeVariations[0];
        if (primaryVariacao) {
        ...
        this.updateSelectedOptions();
      }, 100); // Pequeno delay para garantir que as classes .active sejam atualizadas
    });
  }
  ...
  updateSelectedOptions() {
    ...
    if (selectedOptions.length > 0) {
      ...
      const html = selectedOptions.map(opt => {
        if (opt.image) {
          return `<div class="selected-option-case" data-option="${opt.customTitle}"><strong>${opt.customTitle}:</strong><div class="selected-option"><img src="${opt.image}" alt="${opt.label}" class="option-image"> <span>${opt.label}</span></div></div>`;
        } else {
      ...
      if (savedItem && savedItem.customizations && savedItem.customizations.length > 0) {
        ...
        if (deleteLink) {
          deleteLink.addEventListener('click', (e) => {
            ...
            // Remove do localStorage usando apenas o produtoId
            const savedItems = GofferUtils.storage.get('goffer_customizer');
