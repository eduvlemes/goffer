...
class CartManager {
  ...
  setupObservationsField() {
    ...
    const getCustomizationText = (userText = '') => {
      ...
      // Coleta IDs de produtos que estão atualmente no carrinho
      const cartProductIds = [];
      document.querySelectorAll('[data-produto-id]').forEach(item => {
        const produtoId = parseInt(GofferUtils.getData(item, 'produtoId'));
        if (produtoId) cartProductIds.push(produtoId);
      });
      
      // Filtra apenas os itens que estão no carrinho atual
      const cartItems = savedItems.filter(item => cartProductIds.includes(item.produtoId));
